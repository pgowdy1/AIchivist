...
2. `.claude/rules/backend.md` — C# / .NET rules (if optimizing backend files)
3. `.claude/rules/frontend.md` — Angular rules (if optimizing frontend files)
4. `.claude/rules/testing.md` — test conventions (if optimizing test files)
...
```bash
dotnet test backend/ --verbosity quiet
```
