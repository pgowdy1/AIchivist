...
Each agent gets:
```
Task: [backend-dev | frontend-dev] agent
Prompt: "Fix this bug: $ARGUMENTS

Investigation findings:
[paste debugger agent's report]

Your files to fix:
- [list specific files]

Fix the bug, keep changes minimal. Follow CLAUDE.md conventions.
Do NOT touch files outside your layer."
```

**No contract-first protocol needed** — bug fixes rarely introduce new API contracts. If both agents are spawned, they work in parallel independently.
...
**Backend:**
```bash
cmd.exe /c "dotnet test backend/ --verbosity normal"
```
