...
- **PostgreSQL bundling**: Shipping PostgreSQL as an embedded database with automated initialization, service registration, backup/restore, and clean uninstall
...
3. **PostgreSQL 16** — Embedded database registered as Windows service `AIchivist-PostgreSQL` on port 5433
...
| `installer/scripts/init-postgres.bat` | PostgreSQL init: initdb → service register → start → create DB → restore dump → write config |
| `installer/scripts/uninstall-postgres.bat` | Stops and unregisters the PostgreSQL Windows service |
| `installer/scripts/test-postgres-setup.bat` | Post-install validation: 7 tests covering service, connectivity, data, config, and idempotency |
...
- Data directory: `%LOCALAPPDATA%\AIchivist\data` (survives app updates, user-prompted deletion on uninstall)
...
4. **Clean uninstall**: Remove services, stop processes, and offer to delete user data. Leave no orphaned services or registry entries.
...
7. **Small blast radius**: Keep PostgreSQL isolated — its own port, its own service name, its own data directory. Don't interfere with anything else on the user's system.
...
- Prefer Windows-native approaches (Windows services, %LOCALAPPDATA%, Program Files) over cross-platform patterns.
...
- Consider what happens if the user's machine has antivirus software that might interfere (e.g., blocking service creation).
